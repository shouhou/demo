...
(function(window, undefined) {
	...
	Nearset.prototype = {
		...
		crashTest : function(obj1, obj2) {
			...
			r1 = obj1.offsetLeft + obj1.offsetWidth, t1 = obj1.offsetTop, b1 = obj1.offsetTop + obj1.offsetHeight, l2 = obj2.offsetLeft, r2 = obj2.offsetLeft + obj2.offsetWidth, t2 = obj2.offsetTop, b2 = obj2.offsetTop + obj2.offsetHeight;
		...
		getDis : function(obj1, obj2) {
			//获取元素原心所在的坐标
			var obj1left = obj1.offsetLeft + (obj1.offsetWidth / 2), obj1top = obj1.offsetTop + (obj1.offsetHeight / 2), obj2left = obj2.offsetLeft + (obj2.offsetWidth / 2), obj2top = obj2.offsetTop + (obj2.offsetHeight / 2), x = obj2left - obj1left, y = obj2top - obj1top;
			//圆心垂直距离
		...
		findNearset : function(obj, list) {
			var list = list || [], min = 999999, minIndex = -1;
	...
	function drag(element) {
		...
		//初始化元素，让其相对于父元素确定位置
		var list = document.getElementById('imgWrap').children, Pos = [], len = list.length, index = [];
		...
		for ( i = 0; i < len; i++) {
			...
			list[i].style.left = Pos[i].left + 'px', list[i].style.top = Pos[i].top + 'px';
		}
		//存储所有移动元素的样式left，top值
		this.Pos = Pos, this.elems = list, this.elem = element, this.move();
	}
	...
	drag.prototype = {
		...
		move : function() {
			...
			//鼠标按下当前元素
			var element = this.elem, lists = this.elems, stopPrevent = this.stopPrevent, nearElement;
			//console.log(lists);
			element.style.zIndex = '999', element.style.cursor = 'move', Pos = this.Pos;
			curIndex = this.curIndex,
			//实例构造函数获取对象方法
			Moveto = new Move(), startMove = Moveto.startMove;

			element.onmousedown = function(e) {
				var e = e || window.event, offleft = element.offsetLeft, offtop = element.offsetTop,
				//ie支持event.x event.y,其他浏览器支持event.pageX,event.pageY
				mousex = e.x ? e.x : e.pageX, mousey = e.y ? e.y : e.pageY;
				element.style.zIndex = 9999;
				//阻止默认行为，阻止冒泡
				stopPrevent(e);
				document.onmousemove = function(e) {

					var e = e || window.event, movex = e.x ? e.x : e.pageX, movey = e.y ? e.y : e.pageY;
					stopPrevent(e);
					//元素移动的距离
					var positionx = movex - mousex + offleft, positiony = movey - mousey + offtop;
					element.style.left = positionx + 'px', element.style.top = positiony + 'px';
				...
				document.onmouseup = function() {
					...
					var moveIndex = curIndex(element, lists), activeIndex = curIndex(nearElement, lists);
					if (nearElement) {
						...
						//交换Pos数组存储的位置信息
						var offsetInfo = Pos[activeIndex], offsetInfo1 = Pos[moveIndex];
						Pos[activeIndex] = offsetInfo1, Pos[moveIndex] = offsetInfo;
