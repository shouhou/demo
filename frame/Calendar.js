function PopupCalendar(InstanceName) {
  ...
  this.weekDaySting = ["日","一","二","三","四","五","六"];
	this.monthSting = ["一月", "二月", "三月", "四月", "五月", "六月", "七月", "八月","九月", "十月", "十一月", "十二月"];
	this.Width = 200;
...
function CalendarInit() // /Create panel
{
	...
	for (i = 0; i <= 6; i++) {
		...
		for (j = 0; j < 7; j++) {
			...
			if (i == 0) {
				...
				htmloDayTable += "<td height='20' align='center' valign='middle' style='cursor:hand'";
				htmloDayTable += " onmouseover=CalendarCellsMsOver(this,"
						+ this.instanceName + ")";
	...
	htmloButton += "<button id='"
			+ this.sTODAYBTNID
			+ "' style='width:40%;border:1px solid #BCD0DE;background-color:#eeeeee;cursor:hand'"
...
function CalendarCellsMsOver(cell,oInstance) // / Cell MouseOver
{
	var myCell = cell;
...
function CalendarCellsMsOut(cell,oInstance) // //// Cell MouseOut
{
	var myCell = cell;
...
function CalendarCellsClick(oCell, oInstance) {
	...
	sDateString = sYear + oInstance.separator + CalendarDblNum(sMonth + 1)
			+ oInstance.separator + CalendarDblNum(sDay); // /return
															// sDateString
	if (oInstance.oTaget.tagName.toLowerCase() == "input")
		oInstance.oTaget.value = sDateString;
	CalendarCancel(oInstance);
	if (BaseJs) {
		oInstance.oTaget.value = BaseJs.Utils.format.date(sDateString,
				this.format, 'yyyy-MM-dd');
	}
	BaseJs.element.fireEvent(oInstance.oTaget,'change');
	return oInstance.oTaget.value;
}
function CalendarTodayClick(oInstance) // / "Today" button Change
{
	oInstance.Refresh(new Date());
}
function getDateString(oInputSrc, oInstance, format) {
